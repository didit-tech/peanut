...
var recordStep = function(step, ran, passed, error) {
  ...
  var status = { skipped: true, passed: false };

  if (ran) status.skipped = false;
  if (passed) status.passed = true;
  status.error = error;

  scenario[step.text] = status;
}
...
module.exports = {
  ...
  step: {
    failed: function(step, err) {
      ...
      addToTotals();
      recordStep(step, true, false, err);
    },
