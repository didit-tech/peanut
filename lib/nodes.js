...
File.prototype.setTeardownStepIndex = function() {
  this.vars.teardownStepIndex = this.feature.teardown.steps.length - 1;
};
...
File.prototype.captureScenarioExamples = function(token) {
  var self = this;
  var rows = token[2].split("\n");
  var header = this.cleanRow(rows.shift().trim());

  _(rows).each(function(row) {
    var exampleRow = {};
    _(self.cleanRow(row.trim())).each(function(item, index) {
      exampleRow[header[index]] = item;
    });
...
genNode('Teardown', function(file, token) {
  file.emit('stateChange', 'TEARDOWN');
  file.feature.teardown = {
    lineno: token[1],
    steps: [],
    stepArgs: {}
  };
...
genNode('Serial', function(file, token) {
  file.feature.serial = true;
});

genNode('Timeout', function(file, token) {
  file.feature.timeout = parseInt(token[2].split(' ')[1], 10);
});
...
genNode('Line', function(file, token) {
  ...
  case 'STEP_DESCRIPTION':
    ...
    } else if (file.state === 'TEARDOWN') {
      file.feature.teardown.steps.push(stepToken);
      file.setTeardownStepIndex();
    } else {
...
genNode('Examples', function(file, token) {
