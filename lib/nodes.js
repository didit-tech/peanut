...
File.prototype.capturePyString = function(token) {
	this.vars.pendingPystring = this.formatPyString(token[2]);
};
  ...
  case 'STEP_DESCRIPTION':
    ...
    if (file.prevToken[2] === 'And' || file.prevToken[2] === 'But') {
      ...
      file.vars.prevStepType = file.prevToken[2];
		...
		if (file.vars.pendingPystring) {
			stepToken[4] = 'PYSTRING';
			file.pushStepArg(file.vars.pendingPystring);
			file.vars.pendingPystring = false;
		}
