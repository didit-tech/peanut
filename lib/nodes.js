...
genNode('Teardown', function(file) {
...
genNode('Serial', function(file, token) {
  console.error('The serial directive is deprecated, all tests run serially. Please remove the directive from your features.')
});
...
genNode('Line', function(file, token) {
  ...
  case 'STEP_DESCRIPTION':
    ...
    if (file.state === 'BACKGROUND') {
      ...
      _(file.feature.scenarios).each(function(scenario) {
        scenario.steps.push(stepToken);
      });
    } else {
