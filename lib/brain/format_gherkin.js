...
var formatStep = exports.formatStep = function(step, scenario, file) {
...
var regexReplaceNumbers = function(step) {
  ...
  var pattern = /(\d*\.)?(\d+)/g
  return step.pattern.replace(pattern, function(str, m1, m2, offset) {
    step.args += ', arg';
    var number = m1 ? m1 + m2 : m2;
    stepArgs[offset] = parseFloat(number);
