...
var formatStep = exports.formatStep = function(step, scenario, feature) {
  ...
  return formattedStep(step, stepDef, scenario, opts);
};

var formattedStep = exports.formattedStep = function(step, stepDef, scenario, opts) {
  var _formattedStep = {
    args: step[5],
  ...
  _formattedStep = formatStepOutine(_formattedStep, scenario);
  _formattedStep = formatStepArgs(_formattedStep, stepDef, scenario);
...
var parameterizeStep = exports.parameterizeStep = function(_step) {
  ...
  step[4] = regex;
  step[5] = [''].concat(args).join(', ');

  return step;
};

var formatStepArgs = exports.formatStepArgs = function(step, stepDef, scenario) {
  if (scenario.stepArgs[step.lineno]) {
    if (step.type === 'pystring')
      step.tableOrPyString = scenario.stepArgs[step.lineno].split('\n');
    else
...
var formatStepOutine = exports.formatStepOutine = function(step, scenario) {
