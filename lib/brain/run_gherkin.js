...
var runFeature = exports.runFeature = function(err, data, featureFile, callback) {
...
var runExample = exports.runExample = function(example, scenario, index, exIdx, file, featureVar) {
  ...
  _(clonedScenario.stepArgs).each(function(argVal, argKey) {
    _(example).each(function(val, key) {
      if (_.isArray(argVal)) {
        ...
        clonedScenario._args[argKey] = utils.replaceKeyWithVal(argVal, key, val);
      }
    });
  });
...
var runScenario = exports.runScenario = function(scenario, index, file, featureVar) {
  ...
  var formattedSteps = _(steps).map(function(step) {
    var formattedStep = formatGherkin.formatStep(step, currentScenario, featureVar);
...
var runStep = exports.runStep = function(step, currentScenario, currentFeature, callback) {
      ...
      try {
        timeoutId = setTimeout(function() {
          ...
          if (currentScenario._state !== 'failed') {
            ...
            currentScenario._state = 'failed';
            step.errorHeader = errorHeader(step);
