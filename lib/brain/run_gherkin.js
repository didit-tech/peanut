...
var runScenarios = function(file, callback) {
  var scenarios = file.feature.scenarios;
  if (file.feature.serial || callback) {
    (function next() {
      if (scenarios.length) {
...
var eachFeature = function(objects, callback) {
  if(__peanut__.options.serial) {
    var next = function() {
      if (objects.length) {
        callback(objects.shift(), next);
      }
    }
    next();
  } else {
...
var runTests = exports.runTests = function() {
  ...
  parseGherkin.parseEnvFile(function(err) {
    ...
    parseGherkin.parseFeatures(function(err, parsedFeatures) {
      eachFeature(parsedFeatures, function(parsedFile, next) {
        runFeature(err, parsedFile, function(file) {
          __peanut__.formatter.feature(file.broadcastedFeature);
          featuresRun.push(file.featureVar);
          runScenarios(file, next);
        });
      });
    });
  });
...
var runTestsExceptTag = exports.runTestsExceptTag = function(tag) {
  __peanut__.formatter.runFeatures();
  parseGherkin.parseEnvFile(function(err) {
    parseGherkin.parseStepDefinitions();
    parseGherkin.parseFeatures(function(err, parsedFeatures) {
